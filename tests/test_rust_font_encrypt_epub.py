...
def build_font_encrypt_epub(epub_path: Path, css: str | None = None) -> None:
...
def assert_readable_font_mapping(output_path: Path) -> None:
    ...
    text = re.search(r'<p class="body">(.*?)</p>', html).group(1)
...
def test_rust_font_encrypt_complex_css_remains_on_python_sidecar(tmp_path: Path) -> None:
...
@media screen { .body { font-family: "TestFont"; } }""",
    ...
    request = {
        "taskId": "rust-font-encrypt-complex",
        "taskType": "encrypt_font",
        "inputFiles": [str(input_path)],
        "outputDir": str(output_dir),
        "options": {"target_font_families_by_file": {str(input_path): ["TestFont"]}},
    }
    completed = subprocess.run(
        [
            "cargo",
