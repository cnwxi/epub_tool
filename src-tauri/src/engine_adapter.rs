...
use crate::{
    engine_protocol::v1::{
        engine_response, task_options, FileIssue, FontTargetResult,
        ImageCompressOptions, ImageConversionOptions, RunTaskRequest, TaskEvent, TaskOptions,
        TaskResult, TaskSummary, TaskType,
    },
    FrontendTaskRequest,
};
...
fn frontend_options(options: Option<&TaskOptions>) -> Result<Value, String> {
    ...
    match options.kind.as_ref() {
        ...
        Some(task_options::Kind::ImageCompress(options)) => {
            Ok(image_compress_options(options))
        }
        Some(task_options::Kind::ImageConversion(options)) => {
            Ok(image_conversion_options(options))
        }
...
mod tests {
        ...
        let request = RunTaskRequest {
            ...
            options: Some(TaskOptions {
                kind: Some(task_options::Kind::Font(FontOptions {
                    target_font_families_by_file: [("book.epub".to_string(), FontFamilies {
    ...
    fn converts_rust_event_to_wire_event() {
        ...
        assert_eq!(event.result.expect("finished event result").summary.expect("summary").success, 1);
    }
    ...
    fn rejects_missing_options_kind() {
        ...
        assert_eq!(frontend_options(Some(&empty)).expect("empty options"), json!({}));
    }
}
