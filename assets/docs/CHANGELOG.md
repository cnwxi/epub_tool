...
优化左侧导航的 hover 与 active 状态：当前功能项保持常规底色，叠加由左向右淡出的静态浅暖白标记，并与功能详情页底色呼应；后续延长标记影响范围，将 `38%` 设为过渡点、`62%` 设为透明终点；标记层位于文字与处理引擎状态信息下方，处理引擎状态项同步支持 active 样式，避免当前项与悬停项混淆。<br>
优化左右自定义滚动条：滚动时显示，停止滚动 `900ms` 后自动淡出；内容或窗口尺寸变化只更新 thumb 几何尺寸，不会重新唤醒已隐藏的滚动条。<br>
