...
本项目界面统一采用“轻磨砂玻璃”作为新的视觉方向，但本质仍然是效率工具界面，而不是展示型网站。所有设计决策优先满足以下目标：

- 长时间使用时保持清晰、稳定、低干扰。
- 让用户快速理解“当前做什么、处理到哪里、结果在哪里”。
- 通过局部磨砂质感增强层次，而不是依赖重装饰效果。
- 以 Android 触控设备为主要目标，避免依赖桌面端特效。
...
- 阴影：强于 `glass-medium` 的多层阴影，但仍保持工具的克制感
...
Android 布局优先保证结构稳定，不因屏幕尺寸变化导致层级混乱。

规范：

- 顶部导航与任务切换为稳定结构层，不应随内容一起滚动。
- 工作区独立滚动。
- 小屏优先减少横向密度，不打乱页面骨架。
- 统计卡、设置卡等在中等宽度下允许重排，但不应在小屏宽度下碎裂成过多行。
...
> 以深色导航、暖色内容区为基础，局部引入轻磨砂玻璃质感和轻反馈动画的 Android 效率工具界面。
