...
- 侧边栏：`Brand Navy 900`
- 侧边栏 hover：白色透明高亮并轻微右移；active 保持常规导航底色，仅叠加从左向右淡出的浅暖白标记，与功能详情页底色呼应；标记层必须位于导航文字和状态信息下方
...
- 避免大位移、强弹簧、大面积循环闪烁和复杂旋转；窄幅状态标记可使用低强度循环脉冲。
...
- 右侧工作区独立滚动。
- 桌面端自定义滚动条仅在滚动期间显示，停止滚动 `900ms` 后淡出；它只反映原生滚动位置，不接管拖拽交互。
