...
面向 EPUB 批量处理的桌面工具，技术栈为 Tauri 2 + Vue 3 + TypeScript + Python sidecar。支持五种任务类型：`reformat`、`decrypt`、`encrypt`、`font_encrypt`、`transfer_img`。
...
- **`src-tauri/tauri.conf.json`** — 开发 URL `localhost:5173`，透明窗口（macOS 毛玻璃效果），sidecar 从 `bundle-resources/binaries/` 打包。
...
- **`utils/`** — 五个 EPUB 处理模块（`reformat_epub.py`、`decrypt_epub.py`、`encrypt_epub.py`、`encrypt_font.py`、`transfer_img.py`），各自对外暴露 `run()` 或等价入口，内部使用共享的 `logger` 对象，运行时由 task_runner 替换。另含 `log.py`（`logwriter` 类）。
- **`build_tool/`** — `build_python_sidecar.py`（PyInstaller `--onefile` 构建）、`prepare_bundle_resources.py`（将 sidecar 复制到 `bundle-resources/`）。
- **`test/`** — 本地测试用 EPUB 样本。
...
- `font_encrypt` 的 options 使用 `target_font_families_by_file`（按文件指定字体）和 `target_font_families`（默认字体）。
