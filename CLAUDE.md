...
- **`build_tool/`** — `prepare_ocr_models.py`（准备固定 PaddleOCR 模型）、`build_python_sidecar.py`（PyInstaller `--onefile` 构建）、`prepare_bundle_resources.py`（将 sidecar 复制到 `bundle-resources/`）。
- **`test/`** — 本地测试用 EPUB 样本。
...
- `font_encrypt` 与 `font_decrypt` 的 options 使用 `target_font_families_by_file`（按文件指定字体）和 `target_font_families`（默认字体）；`font_decrypt` 使用内置 `PP-OCRv5_server_rec` PaddleOCR 模型。
